...
{
    $query = mysqli_query($link, "SELECT * FROM tutor_category_tb WHERE id = '$catid'");
...
function getTutoringSubjects($id)
{
    ...
    $message = "";
    $query = mysqli_query($link, "SELECT * FROM tutor_details_tb WHERE id = '$id'");
    $rows = mysqli_fetch_array($query);
    ...
    // first cat..
    $query2 = mysqli_query($link, " SELECT * FROM sub_categories WHERE id = '$subtCateOne' ");
    $subRow1 = mysqli_fetch_array($query2);

    // second cat...
    $query3 = mysqli_query($link, " SELECT * FROM sub_categories WHERE id = '$subCatTwo' ");
    $subRow2 = mysqli_fetch_array($query3);

    // cat three...

    $query4 = mysqli_query($link, " SELECT * FROM sub_categories WHERE id = '$subCatThree' ");
    $subRow3 = mysqli_fetch_array($query4);
...
function computeNearestTutionCentre($lat1, $lon1, $categoryId)
{
    ...
    $cart = array();
    $location = array();
    $result = mysqli_query($link, "SELECT * FROM tution_centers_tb WHERE category_one='$categoryId' "
...
{
    $status = "";
    $result = mysqli_query($link, "SELECT * FROM tution_centers_tb WHERE physical_address = '$usersLocation' ");
...
if (!empty($_POST['latitude']) && !empty($_POST['longitude'])) {
    ...
    $categoryId = 1;
    $result = mysqli_query($link, "SELECT * FROM tution_centers_tb WHERE category_one='$categoryId' AND status = 'Approved' ");
    ...
    $result2 = mysqli_query($link, "SELECT * FROM tution_centers_tb WHERE physical_address = 'Chindo Rd, Lusaka, Zambia' AND  category_one='$categoryId' OR category_two='$categoryId' OR category_three='$categoryId'");
