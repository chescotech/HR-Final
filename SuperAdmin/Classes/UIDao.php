...
class UIDao
{
    ...
    public function addProspectCustomer($type, $name, $contact_person, $location, $cell_one, $cell_two, $landline, $email, $clients_email, $date_of_contact, $item, $no_projects, $last_quoted_amount, $amount_invoiced_to_date, $project_manager, $userId, $prospect, $probability)
    {
        ...
        $res = mysqli_query($this->link, "INSERT INTO customer_prospects_tb(type,name,contact_person,"
    ...
    {
        $result = mysqli_query($this->link, "UPDATE customer_prospects_tb SET type ='$type',name='$name',"
    ...
    {
        $status = "";
        $result = mysqli_query($this->link, "SELECT type FROM customer_prospects_tb WHERE id = (SELECT quote_to FROM quote_tmp_tb where ref_id = '$refId' )");
    ...
    {
        $status = "";
        $query = mysqli_query($this->link, "SELECT * FROM labour_tb WHERE ref_id = '$refNo' ");
    ...
    {
        $result = mysqli_query($this->link, "SELECT * FROM customer_prospects_tb WHERE id = ( SELECT quote_to FROM quote_tmp_tb where ref_id = '$refId'  ) ");
        $rows = mysqli_fetch_array($result);
    ...
    public function checkIfPasswordExsists($userId, $password)
    {
        $status = "";
        $pass = md5($password);
        $result = mysqli_query($this->link, "SELECT * FROM users_tb WHERE password = '$pass' AND id = '$userId'");
    ...
    {
        $pass = md5($password);
        $result = mysqli_query($this->link, "UPDATE users_tb SET password ='$pass' WHERE id = '$userId'");
        return $result;
    }

    public function UploadedBy($user_id)
    {
        $query = mysqli_query($this->link, " SELECT * FROM users_tb WHERE id = '$user_id'");
    ...
    {
        $query = mysqli_query($this->link, " SELECT * FROM customer_prospects_tb ");
        return $query;
    }
    ...
    public function addCostings($item_name, $description, $unit_measure, $latest_price, $latest_price_source, $previous_price, $previous_price_source, $alternate_price, $alternate_price_source, $today, $userId, $supplier_name, $brand)
    {
        ...
        $res = mysqli_query($this->link, "INSERT INTO costings_tb(item_name,description,unit_of_measure,"
    ...
    {
        $res = mysqli_query($this->link, "INSERT INTO quote_tmp_tb(quote_to,po_ref_no,sales_location,sales_person,ref_id,delivery_period,quotation_validity,quotation_for,payment_terms,date_quotation) "
    ...
    {
        $res = mysqli_query($this->link, "INSERT INTO quote_items_temp_tb(item_description,dimensions,qty,"
    ...
    {
        $res = mysqli_query($this->link, "INSERT INTO labour_tb(labour_description,labour_charge,estimated_days,"
            . "ref_id) "
    ...
    {
        $res = mysqli_query($this->link, "SELECT * FROM pricing_tb");
        return $res;
    }
    ...
    public function addPricing(
        ...
        $res = mysqli_query($this->link, "INSERT INTO pricing_tb(job_type,description,item_type,"
    ...
    public function updatePricing(
        ...
        $result = mysqli_query($this->link, "UPDATE pricing_tb SET job_type ='$job_type',description='$description',"
    ...
    {
        $result = mysqli_query($this->link, "UPDATE costings_tb SET item_name ='$item_name',description='$description',"
